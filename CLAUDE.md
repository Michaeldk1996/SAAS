...
- **Break/Hold heatmap — window 24 months, axis bucketed (founder ruling 2026-09-01, TEN-107 comment 0f9961ce: "24 window and shipping bucketed. push it online").** The `holdbreak.json` shard (built by `build-holdbreak.js` from the harvested historical point-by-point in `apitennis-styles-cache/`) rolls up SERVE hold% and RETURN break% over a **24-month** window (not the app's 52-week default — a window does not transfer between measurements), on a **bucketed** position axis: **early = the server's 1st–2nd service game in the set, mid = 3rd–4th, late = 5th+** — keyed on the *server's service-game ordinal within the set*, never the raw game number (which alternates server). Columns are set 1/2/3/4+; surfaces all/hard/clay/grass. hold = `serve_winner === player_served`, break = the complement (so per cell hold% + break% = 100). Source is pbp only — box scores and Tennis-Abstract splits have no game position and cannot build it. The shard emits `{pct, n}` on every cell and never drops data; `meta.sampleFloor` (currently 20) is the *display* floor to grey thin cells — not a founder-locked threshold, adjustable without a regen. Test: `holdbreak.json` `meta.windowMonths === 24` and `meta.buckets === ['early','mid','late']`; any cell where `serve.hold.pct + return.break.pct` for the same (surface,set,bucket) ≠ 100 (±0.1) is wrong; a raw-game-number axis or a 52-week window is wrong.
