...
- **Active-pill text (`--active-pill-text: #fff`, founder ruling 2026-08-01).** Pure white `#fff` on *text* is reserved for the label of an **active segmented-control pill on a brand-tint fill** — nothing else in the product paints text `#fff`; body/label text stays `#e7e9ee`. The export settles it: the active `span.seg` leaf computes `rgb(255,255,255)`/700 on the tinted fill in both matches-upcoming and matches-completed states, while the inactive sibling stays `#5b6880` — so the brighten-to-white is a deliberate accent, not a `#e7e9ee` artifact, and the default resolution rule (export wins on measurable values) keeps it. Test: `#fff` text is valid only on an active segmented-control pill over a brand-tint fill; anywhere else text is `#fff`, it's wrong — use `#e7e9ee`.
