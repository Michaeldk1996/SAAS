...
**Build:** locked layout **9e**, box set re-locked **2026-09-17**. The two `.dc.html` files in this folder are copies of `Player Profile LOCKED.dc.html` and `Player Stat Boxes LOCKED.dc.html` — the frozen versions. Where this README and the files disagree, the files win.
...
| # | key | Title | Headline | Size | Support | Icon path |
|---|---|---|---|---|---|---|
| 1 | `career` | Career record | `85–43` | 26px | `66.4% all-time · by surface and by season` | `M6 4h8v3a4 4 0 01-8 0V4ZM10 11v3M7.5 16.5h5` |
| 2 | `season` | Calendar record | `46–13` | 26px | `2026 season · all surfaces · 0 titles` | `M4 3h12v14H4zM4 7h12M8 3v14` |
| 3 | `tourn` | Record per tournament | `+4.2u` (suffix colour `#3dd68c`) | 30px | `Cincinnati · best event · 14–4 · 78%` | `M4 5h12v4a6 6 0 01-12 0V5ZM10 15v2M7 18h6` |
| 4 | `speed` | Court speed record | `Fast courts` | 22px | `72% · 31–12 · 43 matches` | `M3 14c3-6 11-6 14 0M10 4v3M6 6l2 2M14 6l-2 2` |
| 5 | `splits` | Draw record | `Other Tours` | 20px | `best split · 75.0% · 45–15 · 60 matches` | `M4 15V9M8 15V5M12 15v-4M16 15V7` |
| 6 | `styles` | Matchup record | `74%` | 30px | `Attacking Baseliner · his best archetype · 20–7` | `M10 3v14M4 7l6-4 6 4v6l-6 4-6-4Z` |
| 7 | `market` | Market edge | `−1.87%` | 26px | `flat-stake yield · 537 priced · tour −3.79%` | `M3 13l4-5 3 3 4-6 3 4M3 17h14` |
| 8 | `profile` | Live trading | `8–19` | 30px | `from a set down · 29.6% · 6.4pp below tour` | `M4 15V9M8 15V5M12 15v-4M16 15V7` |

**Tile copy rule (locked).** Every tile reads **figure → box name → detail line**: the headline is the number or band that matters, the title names the box, the support line carries the qualifier and the sample. Headline size is set per box by the `size` field above — do not derive it from string length. `tourn` is the only box with a coloured headline suffix.

The page can override `headline` and `support` per key through `vals`; anything it doesn't send falls back to the defaults above. `barW` exists on every box object and is **not rendered** — dead field, ignore it.
...
Subtitles (modal header, under the title): career `Record by surface and season, and his ratings against the field` · season `Where in the calendar his results sit · 678 matches · 2016–2026` · tourn `Career win–loss at every event he has played` · speed `Win rate by court pace band` · splits `Record and win rate by level, format, round and opponent` · styles `Win rate by opposing archetype · minimum 5 matches` · market `How the market has priced him, and what backing him flat has returned` · profile `In-play states and how he plays from them`.

**In-modal tab rows (locked).** Three modals open on a tab row directly under the header (`bg #0a0d13; border rgba(255,255,255,0.09); radius 10; padding 3; width fit-content; margin-bottom 18px`; item `padding 7px 14px; radius 8; 12px`; on `700 #e7e9ee bg rgba(91,155,255,0.16)`):

| Modal | Tabs | Default |
|---|---|---|
| Career record | `Record` · `Ratings` | Record |
| Market edge | `Match winner` · `Derived lines` | Match winner |
| Live trading | `Ratings` · `Live trading` · `Lines` | Ratings |

Career record and Draw record also carry a **grain** segmented control in the modal header, right of the title, `Career | Last 52` (`padding 5px 12px; radius 7; 11px`). Calendar record keeps its own `Calendar | Streaks` row, Court speed its surface chips.
...
Groups (always these four, this order, these labels): **Level** Grand Slams / Masters / Other Tours · **Format** Best of 5 / Best of 3 · **By round** Finals / Semi-finals / Quarter-finals / Early rounds · **Opponent** vs Righties / vs Lefties / vs Top 10 / vs Top 50. **There is no Surface group** — it was removed on 2026-09-17 because Career record already carries by-surface; do not add it back. Group label = eyebrow spanning the grid, `padding 14px 0 6px; border-top rgba(255,255,255,0.06)`. Row `padding 7px 0; border-top rgba(255,255,255,0.04)`, label `12.5px/700 nowrap`.
...
### 5.9 Live trading (modal)

Segmented `Ratings | Live trading | Lines` (the third tab holds the derived-line coverage table, `Best of 3 | Best of 5`) (`bg #0a0d13; radius 10; padding 3`; item `7px 14px; radius 8; 12px`).
...
Placeholder copy (exact, from the locked build):

1. **Strongly surface-dependent** — `82.6% on clay (19-4) against 63.6% on hard (14-8) — a 19pp swing between his best and worst surface, where the typical player swings 12.2pp. Nearly 1.6x the tour's normal surface dependency.` — amber: icon `M10 2.5a5 5 0 00-3 9v2.5h6V11.5a5 5 0 00-3-9ZM8 17h4`, bg `rgba(232,168,78,0.12)`, border `rgba(232,168,78,0.32)`, colour `#e8a84e`
2. **Undervalued by the market as a favourite** — `Wins 80.8% when priced as the favourite (21-5) where the closing price implied 72.3% — beating the market by +8.5pp, against a tour median of +2pp. The market has been pricing him short.` — green: icon `M6 4h8v3a4 4 0 01-8 0V4ZM10 11v3M7.5 16.5h5`, bg `rgba(61,214,140,0.12)`, border `rgba(61,214,140,0.32)`, colour `#3dd68c`
3. **Standout solid baseliner profile** — `Solid Baseliner score of 93 against a tour average of 50, and 16 points clear of his own next dimension (Attacking Baseliner, 77). A clearly one-dimensional style signature.` — green, same icon and tokens as 2

Insight cards carry an icon **border** as well as a fill (`iconBd`), unlike the earlier build.
...
| File | What it holds |
|---|---|
| `Player Profile.dc.html` | **copy of `Player Profile LOCKED.dc.html`** — page frame, header, ribbon, ledger, insights, tournament detail, match panel, match sheet; page logic |
| `Player Stat Boxes.dc.html` | **copy of `Player Stat Boxes LOCKED.dc.html`** — the eight boxes, every modal, hold/break heatmap, in-modal match sheet; box logic |
