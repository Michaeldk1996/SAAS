...
- **The card carries no status line; the price-history box carries book, time and history** (TEN-270, founder 2026-09-24T10:16Z — supersedes "every pre-match Now shows its book and its time on the card face" and the one-line native tooltip). Tests:
  ...
  - **Sources:** Bet105 history from the `price_history(card_key)` RPC (poller archive + stream, deduped on side + Kibl time + price; same-price re-inserts are not changes). Loaded on open, one card, cached for the page session. Never interpolated; a missing value is a dash.
