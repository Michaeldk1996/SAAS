...
- **The card carries no status line; the price-history box carries book, time and history** (TEN-270, founder 2026-09-24T10:16Z — supersedes "every pre-match Now shows its book and its time on the card face" and the one-line native tooltip). Tests:
  ...
  - **Sources, per book and state** (founder 2026-09-24T23:09Z for bet365). Test: `PriceHistoryBox.cardData(m).source` in `test-ten270-price-history-box.mjs`.
    ...
    - **bet365 upcoming:** `m.oddsMovement` from the lazy odds shard (`ensureOddsMovement`), every distinct price the 15-min capture sees at bet365's own tick times, with the source line under the header.
    ...
    - **History ends at the card's start** (`odds-card-state` `startTs`, the clock the Close is judged against): no row after it, and a gap is cut at it. A vendor row inserted after the off without a live flag never shows above the Close.
