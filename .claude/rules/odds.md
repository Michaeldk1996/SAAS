...
- **Schedule:** cron `7,22,37,52 * * * *`, or the pg_cron dispatcher in `oddspapi-postmatch-pinger.sql` (not applied) — exactly one of the two; applying the pinger deletes the `schedule:` block in the same change.
- **The loop wins the key.** An oddspapi call starts only (a) inside :05–:14 of a quarter hour with 30 s to spare, and (b) when `loop_idle()` says the odds loop's iteration is over: no `odds-now.yml` run in progress, or its `chore(odds): capture tick` commit is on main since both the quarter hour and that run's start. Checked before every call; anything unreadable counts as busy.
