...
- **Verdicts (a checked book with no line):** written as `meta` without `books` — never a zero, never another book's price. Pinnacle +30s: a 404 → checked, not priced; the fixture mapper's miss → note "not listed by Oddspapi". odds-api.io books: every configured book on every card; a card dated before the recorder began (2026-09-26) → note "not recorded — our recording began 26 Sep" (never "not priced": we were not recording). Bet105: no rows → checked, not priced. The pipeline ships a verdict-only chart in the shard.
- **Display** (`bsp-consult-dashboard.html` `buildOddsSection`, tests in `test-ten295-odds-chart.mjs`, which executes the real renderer):
  - Chart-line chips and the per-book table are grouped **Sharp** then **Soft**, each with a group header. The group of a configured book is fixed by `AODDS_CONFIG` (the locked spec: Pinnacle +30s and Bet105 Sharp, Superbet and Betfair Exchange Soft), whatever a writer stored.
  - **Every configured book has a row on every card, in the fixed order** Pinnacle +30s, Bet105 | Superbet, Betfair Exchange (recorded by us); legacy/other books follow. A book with no line keeps its slot and shows dashes and its verdict: the writer's note, else "not priced for this match" when the source was checked, else "not checked yet". The subheading reads **"X of Y books priced"**, Y = the configured books (4), X = those with a line. A card with no line at all keeps the reduced view and still gets this table.
