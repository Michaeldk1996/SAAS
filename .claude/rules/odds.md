...
- **Display** (`bsp-consult-dashboard.html` `buildOddsSection`, tests in `test-ten295-odds-chart.mjs`, which executes the real renderer):
  ...
  - **The page's start is `aOddsStartMs(m)`** (founder 2026-09-26: comment 5dafce2b "cards switch to started at the real start time", card f8b311c9 "fix the model display gate") — most proven first: the card state's ACTUAL start (`odds-card-state` `startTs`), `m.startTs`, the joined Oddspapi fixture's UTC start (the shard's `startTime`), else `date`+`time` in the account zone (Europe/Berlin, DST-aware — `cardStartMs()`). It cuts every drawn line, the Key Factors mini-chart, the upcoming/"no recent data" state, **and the edge-model display gate** (`renderEdgeModel`: "Match in play." / the 30-h window; display only, `runModel` untouched). Test: a card at 14:00 (CEST) with a tick at 13:00Z draws no 13:00Z point; a card-state start of 10:00Z beats the schedule; the gate reads `aOddsStartMs`. Measured before the fix (26 Sep board): 15 in-play legacy bet365 points drawn on 4 finished cards → 0 after.
...
**The board card's own close** (`m.closingOdds`, `bsp-pipeline.js` `closeCutMs`; TEN-124 / TEN-179; founder 2026-09-26 card f8b311c9) is cut at, most proven first: the card state's ACTUAL start (`odds-card-state.json` `startTs`, keyed by the card-state key — the pipeline's `ocsMatchKey` is the page's, character for character), `m.startTs`, the joined Oddspapi fixture's UTC start, the in-play onset − 1 ms; else a dash. A carried close later than that cut is re-derived (self-heals). Test: Harris v Kovacevic (actual start 10:31:59Z, schedule 10:35Z) closes at the last bet365 prices before 10:31:59Z — 1.47 / 2.62 — never the 10:32:14Z in-play 1.50.
