...
- **Start cut (review 2026-09-26):** every chart line ends at the card's ACTUAL start — `odds-card-state` `startTs` — else the source's scheduled start (Oddspapi fixture `startTime`, odds-api.io `start_at`). No point after it is ever written, held points included (`chart_series.card_start` / `put_chart(cut_at=)`). Test: a Pinnacle +30s tick, or an odds-api.io tick before the vendor's live flip, after the start is not in the chart. On a late start the scheduled fallback can drop real pre-start points — a missing point, never an in-play one.
- **Completed cards:** the 3-hourly Oddspapi leg captures a completed card only when it holds no movement at all (a card with the frozen bet365 series is not re-targeted). A completed Bet105 card is read until one read lands at or after its start, then never again.
