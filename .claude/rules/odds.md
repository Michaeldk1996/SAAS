...
**The bet365 post-match archive** (TEN-270, founder 2026-09-24/25: "Find a way to archive it straight after the game"; "Separate 15-min job … the live price loop always wins the key"). `archive-oddspapi-raw.py postmatch`, workflow `oddspapi-postmatch.yml`. Tests in `test-oddspapi-postmatch.py`, each run against a mutant:
...
- **Same path as the daily raw archive** (`object_path()`), uploaded without overwrite; the daily `held_objects()` then skips it. A fixture the bucket holds is never pulled again by either job.
- **Schedule:** cron `7,22,37,52 * * * *` (plus the pg_cron dispatcher in `oddspapi-postmatch-pinger.sql`, not applied yet). The odds loop ticks at :00/:15/:30/:45, so **no oddspapi call is made outside :05–:14 of a quarter hour** — a delayed run waits for :05 or stops.
- **429:** one short backoff (10 s), one retry; a second 429 stops the run with exit 0 and the next run resumes. Every 429 is counted in the log and in `_meta/postmatch-last-run.json`.
- **404:** recorded in `_meta/postmatch-state.json`; after 3 runs with a 404 the fixture is not asked for again.
