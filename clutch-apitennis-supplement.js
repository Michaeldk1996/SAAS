...
//
// Output: clutch-supplement.json (sidecar) AND, with --merge, appended into a
// staged clutch-rating.json. Reads are metered — one get_fixtures per player.
// =================================================================
const fs = require('fs');
...
async function harvest(key, playerKey) {
  ...
  const a = { matches: 0, bpFaced: 0, bpSaved: 0, brkChances: 0, brkMade: 0, tbPlayed: 0, tbWon: 0, decPlayed: 0, decWon: 0 };
  for (const f of R) {
...
(async () => {
  ...
  for (const t of targets) {
    ...
    let a;
    try { a = await harvest(key, t.key); }
    catch (e) { console.log(`  ERR ${t.name}: ${e.message}`); continue; }
    const comp = {
      bpSavedPct: a.bpFaced >= MIN_BP_FACED ? +(a.bpSaved / a.bpFaced * 100).toFixed(1) : null,
      bpConvPct: a.brkChances >= MIN_BP_CHANCE ? +(a.brkMade / a.brkChances * 100).toFixed(1) : null,
      tbWinPct: a.tbPlayed >= MIN_TB ? +(a.tbWon / a.tbPlayed * 100).toFixed(1) : null,
      decWinPct: a.decPlayed >= MIN_DEC ? +(a.decWon / a.decPlayed * 100).toFixed(1) : null,
    };
    const parts = [];
    ...
    rows.push({
      ...
      confidence: have >= 4 ? 'med' : have === 3 ? 'med' : 'low', // api-provisional caps at 'med'
      provisional: true, source: 'api-tennis',
      ...comp,
      sample: { matches: a.matches, bpFaced: a.bpFaced, bpChances: a.brkChances, tbPlayed: a.tbPlayed, decPlayed: a.decPlayed },
      alreadyInPool: existingNames.has(abbrName),
    });
    console.log(`  ${t.full.padEnd(26)} idx=${clutchIndex == null ? 'null' : clutchIndex}  BPsv=${comp.bpSavedPct} BPcv=${comp.bpConvPct} TB=${comp.tbWinPct} DEC=${comp.decWinPct}  [${have}/4, ${rows[rows.length-1].confidence}]  n=${a.matches}`);
  }

  const supp = {
    generatedAt: new Date().toISOString(),
    source: 'api-tennis per-match statistics (Tour + Challenger) — provisional under-pressure for players below TML tour floors',
    method: 'Same four ATP Under-Pressure components as clutch-rating.js; clutchIndex = mean percentile vs the existing clutch-rating.json pool.',
