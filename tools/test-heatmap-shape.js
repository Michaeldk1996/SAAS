...
check('item 1 · Hold|Break is a segmented control and Hold is the default', () => {
  ...
  assert.ok(/font-weight:700/.test(seg) && /rgba\(91,155,255,0\.16\)/.test(seg),
...
check('item 3 · column heads are Global and S1-S5, not "Set 1" and "All"', () => {
  ...
  const grid = html.slice(html.indexOf('class="pp2-hb-grid"'));
  const head = grid.slice(0, grid.indexOf('font-size:13.5px;font-weight:700;color:#e7e9ee'));
...
check('item 4 · the legend note is BELOW the grid, at the export\'s size and colour', () => {
  const html = sheet(I);
  const gridAt = html.indexOf('class="pp2-hb-grid"');
  const noteAt = html.indexOf('font-size:11.5px;color:#4b5672;line-height:1.6');
  assert.ok(noteAt > -1, 'the note is not at the export\'s 11.5px/#4b5672/1.6 (:1174)');
...
check('item 5 · ONE context chip, in the export\'s shape', () => {
  ...
  assert.ok(/font-size:9\.5px;font-weight:600;letter-spacing:0\.14em;text-transform:uppercase;color:#5b6880;background:#06070a/.test(html),
...
check('item 6 · subject name in the blue accent, overall pill right-aligned', () => {
  ...
  assert.ok(/font-size:15px;font-weight:700;color:#5b9bff/.test(html),
    'the subject name is not 15px/700 in the blue accent');
  // :1145 — the pill
  assert.ok(/font-size:12px;font-weight:700;letter-spacing:0\.1em;text-transform:uppercase;color:#e8ecf4/.test(html),
...
check('item 9 · 5-9 muted at 0.07, under five plain with a raw label, n=0 a dash', () => {
  ...
  const rawIdx = grid.indexOf('rgba(255,255,255,0.03)');
...
check('item 10 · row sub-labels are the blue accent, not grey', () => {
  ...
  assert.ok(/font-size:9\.5px;color:#5b9bff/.test(html),
...
check('KEEP · surfaces use the SAME segmented chrome as Hold|Break', () => {
  const html = sheet(I);
  const seg = 'background:#06070a;border:1px solid rgba(255,255,255,0.09);border-radius:9px;padding:3px;';
...
function scoreAgainst(M) {
  ...
  }
  {
    const gridAt = html.indexOf('class="pp2-hb-grid"');
    const noteAt = html.indexOf('font-size:11.5px;color:#4b5672;line-height:1.6');
    if (noteAt < 0) throw new Error("the note is not at the export's 11.5px/#4b5672/1.6");
  ...
  if (!/font-size:15px;font-weight:700;color:#5b9bff/.test(html))
  ...
  if (!/font-size:9\.5px;color:#5b9bff/.test(html))
