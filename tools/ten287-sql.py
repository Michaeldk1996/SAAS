#!/usr/bin/env python3
"""TEN-287 — run a list of SQL statements against Supabase via the Management API.

Used for read-only probes, the silent backtest, and the bot install. Results are
ENCRYPTED to tools/ten287-pub.pem before upload (this repo is public; Bet105 prices
...
def main():
    steps = json.load(open(os.environ["TEN287_STEPS"]))
    out, failed = {}, False
    for st in steps:
        if "sleep" in st:
        ...
        if "vault_from_env" in st:
            ...
            q = (f"delete from vault.secrets where name = '{st['vault_name']}'; "
                 f"select vault.create_secret(${tag}${val}${tag}$, '{st['vault_name']}', 'TEN-287 odds-api.io recorder') is not null as stored")
