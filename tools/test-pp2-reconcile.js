...
mustFail('[neg] the head check would catch the shipped zero bottom-padding', () => {
  const html = 'color:var(--periwinkle);text-align:right;">Hard<';
  assert(/color:var\(--periwinkle\);padding-bottom:11px;[^>]*>Hard</.test(html), 'the Hard head has no padding');
});
...
mustFail('[neg] the group-row check would catch the shipped year eyebrow', () => {
  const shipped = '<div style="letter-spacing:0.12em;text-transform:uppercase;color:var(--label);' +
...
mustFail('[neg] the scroll-cap check would catch the shipped uncapped list', () => {
  const shipped = '<div style="background:var(--surface-inner);padding:13px 15px;">';
